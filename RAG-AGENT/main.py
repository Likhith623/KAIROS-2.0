...
def call_gemini_ai_web_only(query, conversation_context=""):
    ...
    prompt = (
        ...
        f"2. Use bullet points, numbered lists, or paragraphs as appropriate for clarity. But dont add too much spaces after each paragraph.\n"
        ...
        f"5. If the question is about a person, summarize the most relevant profiles or pages, including links. And list out all the links at the bottom of the response for reference.\n"
...
CORS(app, 
     origins=[
         "https://agent.vidyarang.online",  # ✅ Your production frontend
         "http://127.0.0.1:3000",           # ✅ For local testing
         "https://*.run.app" 
         "http://34.134.139.155:8080",
         "http://34.134.139.155",
         "https://34.134.139.155",
         "http://localhost:3000",
         "https://ai-agent-frontend-dev-816726965449-us-central1.run.app",
         "https://ai-agent-frontend-prod-816726965449-us-central1.run.app",
         "https://ai-agent-frontend-*.run.app",
         "https://frontend-*.run.app",
         "https://*-816726965449-*.run.app",
         "https://*.run.app"
       # Any deployment with your project ID
     ], 
     allow_headers="*",
...
def handle_universal_search():
    ...
    try:
        ...
        doc_just_uploaded = (
            conversation and
            conversation['id'] in document_usage_tracker and
            document_usage_tracker[conversation['id']]
        )

        if doc_just_uploaded and is_document_summary_query(query) and docs and len(docs) > 0:
            print("📝 Detected document summary query and document is uploaded (first query after upload)!")
            ...
            ai_response = summary
            # Reset the flag so only the first query after upload triggers summary
