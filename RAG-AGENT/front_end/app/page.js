...
export default function ChatInterface() {
  ...
  const [docJustUploaded, setDocJustUploaded] = useState(false);
...
useEffect(() => {
  ...
  if (savedEmail) {
    ...
    setIsSignedIn(true);
    startNewConversationOnLogin(savedEmail); // Always start a new conversation on login
    loadConversations(savedEmail);
  }
}, []);

const startNewConversationOnLogin = async (email) => {
  try {
    ...
    if (response.data.conversation) {
      ...
      setMessages([]); // Clear chat window, do NOT delete old conversations
    }
  } catch (error) {
...
const signIn = async () => {
  ...
  setIsSignedIn(true);
  await startNewConversationOnLogin(userEmail); // Always start a new conversation on sign in
...
// ...existing code...
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        setDocJustUploaded(true);
        setHasDocumentUploaded(true); 
  ...
  const sendMessage = async () => {
    ...
    // FIX: Only reset docJustUploaded here, after the first summary-pattern query
  if (mode === 'document_summary' && docJustUploaded) {
    setDocJustUploaded(false);
  }
...
const getLoadingMode = () => {
  ...
  // Only show summarizing animation if docJustUploaded AND hasDocumentUploaded
  if (
    input &&
    summaryPatterns.some((pat) => pat.test(input)) &&
    docJustUploaded &&
    hasDocumentUploaded
  ) {
